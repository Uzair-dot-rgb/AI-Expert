...
def generate_image_from_text(prompt):
    ...
    for model in MODELS:
        url = f"https://router.huggingface.co/hf-interference/models{model}"
        ...
        for _ in range(3):
            ...
            if r.status_code == 200 and "application/json" not in ct:
                try:
                    ...
                    last_err = f"Requets failed with stauts code 200: Could not decode image bytes: {e}"
                    break
...
def post_process_image(image):
    ...
    return image.filter(ImageFilter.GuassianBlur(radius = 2))
