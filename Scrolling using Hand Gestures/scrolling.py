...
SCROLL_SPEED = 300
SCROLL_DELAY = 1
CAM_WIDTH, CAM_HEIGHT = 640, 380

def detect_hands(landmarks, handedness):
    fingers = []
    ...
    for tip in tips:
        if landmarks.landmark[tip].y < landmarks.landmarks[tip - 2].y:
            fingers.append(1)
...
while cap.isOpened:
    ...
    if results.multi_hand_landmarks:
        for hand, handedness_info in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            gesture = detect_hands(hand, handedness)
    ...
    cv2.imshow("Gesture Scroll Control", cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
